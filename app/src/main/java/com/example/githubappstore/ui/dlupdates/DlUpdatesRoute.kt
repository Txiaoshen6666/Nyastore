...
fun DlUpdatesRoute(dlVm: DownloadViewModel = viewModel(), upVm: UpdatesViewModel = viewModel()) {
    ...
    PullToRefreshBox(modifier = Modifier.fillMaxSize(), isRefreshing = refreshing, state = refreshState, onRefresh = { upVm.scan() }) {
        Column(modifier = Modifier.fillMaxSize().padding(horizontal = 16.dp)) {
            ...
            when (val s = upState) {
                ...
                is UpdatesViewModel.UiState.Result -> { val candidates = s.candidates; val updatable = candidates.filter { it.hasUpdate }; val upToDate = candidates.filter { !it.hasUpdate }; Text("检测到 ${candidates.size} 个受支持应用 · 可更新 ${updatable.size}", style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier.padding(bottom = 6.dp)); StaggeredLazyColumn(verticalArrangement = Arrangement.spacedBy(10.dp), contentPadding = PaddingValues(bottom = 24.dp), modifier = Modifier.fillMaxSize()) {
