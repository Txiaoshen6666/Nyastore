...
fun HomeRoute(homeVm: HomeViewModel = viewModel(), dlVm: DownloadViewModel = viewModel()) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(horizontal = 16.dp)) {
        ...
        PullToRefreshBox(modifier = Modifier.fillMaxWidth().weight(1f), state = refreshState, isRefreshing = refreshing, onRefresh = { refreshing = true; if (isSearching) homeVm.reloadSearch() else homeVm.load(); refreshing = false }) {
